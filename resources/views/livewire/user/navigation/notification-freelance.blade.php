<div>
    ...
    <a x-data="{ linkHover: false }" href="#"
        ...
        <div class="flex items-center">
            <svg class="w-8 h-8 bg-blue-600 bg-opacity-20 text-blue-600 px-1.5 py-0.5 rounded-full" fill="none"
            ...
            <div class="ml-3 text-sm">
                <p class="font-bold capitalize" :class=" linkHover ? 'text-red-600' : 'text-gray-800'">
                    Order
                    Completed</p>
                <p class="p-2 text-xs text-gray-600 truncate dark:text-gray-200">{{$notification->content}}</p>
            </div>
        </div>
        <span class="flex-1 ml-2 text-xs font-bold text-gray-600 dark:text-gray-200">
            <span>{{$notification->created_at->DiffForHumans()}}</span>

            <button x-show="linkHover" type="button" class="flex items-center px-2 py-1 pl-0 space-x-1">
